...
function fetchAraxes(product: string){
    const request = axios.get(`https://araxes.ee/wp-admin/admin-ajax.php?action=woodmart_ajax_search&number=20&post_type=product&query=${product}`).then((data)=>{
...
async function scrapeAraxes(req: Request, res: Response, next: NextFunction) {
    ...
    product.price = product.price.match(/\d+/g).join('.'); //araxes sends price as span
